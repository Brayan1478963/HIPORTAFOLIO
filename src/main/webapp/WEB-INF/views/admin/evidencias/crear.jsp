...
<title>Nueva Evidencia — Admin</title>
...
<header class="admin-topbar">
  <div class="topbar-left"><button id="sidebar-mobile-btn" class="topbar-btn"><i class="bi bi-list fs-5"></i></button>
    <div class="topbar-breadcrumb"><a href="${pageContext.request.contextPath}/admin/evidencias?accion=listar">Evidencias</a><span class="sep">/</span><span>Nueva</span></div></div>
...
<main class="admin-content">
  <div class="page-header"><div><span class="page-tag">// GESTIÓN</span><h1 class="page-title">NUEVA EVIDENCIA</h1></div></div>
  ...
  <div class="admin-panel" style="max-width:700px">
    <div class="admin-panel-header"><div class="admin-panel-title"><i class="bi bi-camera-fill"></i>DATOS DE LA EVIDENCIA</div></div>
    <div class="admin-panel-body">
      <form action="${pageContext.request.contextPath}/admin/evidencias" method="post" enctype="multipart/form-data">
        ...
        <div class="hp-form-grid cols-2">
          <div class="hp-field">
            ...
            <select name="idSemana" class="hp-select" required>
              ...
              <c:forEach var="s" items="${semanas}"><option value="${s.idSemana}">S${s.numeroFormateado} — ${fn:substring(s.titulo,0,35)}</option></c:forEach>
            </select>
          </div>
          <div class="hp-field">
          ...
          <div class="hp-field hp-form-col-full">
            ...
            <input type="text" name="titulo" class="hp-input" required maxlength="300" placeholder="Título de la evidencia">
          </div>
          <div class="hp-field hp-form-col-full">
            <label class="hp-label">Descripción</label>
            <textarea name="descripcion" class="hp-textarea" rows="3" placeholder="Descripción de la evidencia..."></textarea>
          </div>
          <div class="hp-field">
            <label class="hp-label">Fecha</label>
            <input type="date" name="fechaEvidencia" class="hp-input">
          </div>
          <div class="hp-field">
            <label class="hp-label">Archivo adjunto</label>
            <input type="file" name="archivo" class="hp-input" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif,.zip">
            <span class="hp-field-hint">Máx. 10 MB — PDF, Word, imágenes, ZIP</span>
          </div>
        </div>
