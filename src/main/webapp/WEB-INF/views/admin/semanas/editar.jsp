...
<title>Editar Semana — Admin</title>
...
<header class="admin-topbar">
  <div class="topbar-left"><button id="sidebar-mobile-btn" class="topbar-btn"><i class="bi bi-list fs-5"></i></button>
    <div class="topbar-breadcrumb"><a href="${pageContext.request.contextPath}/admin/semanas?accion=listar">Semanas</a><span class="sep">/</span><span>Editar</span></div></div>
...
<main class="admin-content">
  <div class="page-header"><div><span class="page-tag">// GESTIÓN</span><h1 class="page-title">EDITAR SEMANA ${semana.numeroFormateado}</h1></div></div>
  ...
  <div class="admin-panel" style="max-width:700px">
    <div class="admin-panel-header"><div class="admin-panel-title"><i class="bi bi-pencil-square"></i>SEMANA ${semana.numeroFormateado}</div></div>
    <div class="admin-panel-body">
      <form action="${pageContext.request.contextPath}/admin/semanas" method="post">
        <input type="hidden" name="accion" value="actualizar">
        <input type="hidden" name="id" value="${semana.idSemana}">
        <div class="hp-form-grid cols-2">
          <div class="hp-field">
            ...
            <select name="numero" class="hp-select" required>
              <c:forEach begin="1" end="16" var="n"><option value="${n}" ${n eq semana.numero ? 'selected':''}">Semana ${n < 10 ? '0':''}${n}</option></c:forEach>
            </select>
          </div>
          <div class="hp-field">
            <label class="hp-label">Unidad <span class="required">*</span></label>
            <select name="idUnidad" class="hp-select" required>
              <c:forEach var="u" items="${unidades}"><option value="${u.idUnidad}" ${u.idUnidad eq semana.idUnidad ? 'selected':''}>Unidad ${u.numeroARomano}</option></c:forEach>
            </select>
          </div>
          <div class="hp-field hp-form-col-full">
            <label class="hp-label">Título <span class="required">*</span></label>
            <input type="text" name="titulo" class="hp-input" required value="${semana.titulo}">
          </div>
          <div class="hp-field hp-form-col-full">
            <label class="hp-label">Descripción</label>
            <textarea name="descripcion" class="hp-textarea" rows="3">${semana.descripcion}</textarea>
          </div>
          <div class="hp-field hp-form-col-full">
            <label class="hp-label">Objetivo</label>
            <textarea name="objetivo" class="hp-textarea" rows="3">${semana.objetivo}</textarea>
          </div>
          <div class="hp-field">
            <div class="hp-toggle-wrap">
              <label class="hp-toggle"><input type="checkbox" name="estado" ${semana.estado ? 'checked':''}><span class="hp-toggle-slider"></span></label>
              <span class="hp-toggle-label">Semana Activa</span>
            </div>
          </div>
        </div>
